...
DiscV5MaskedMessage::DiscV5MaskedMessage(const shared_ptr<const DiscV5MaskedMessage> masked_msg)
    : SocketMessage(masked_msg->getSessionHandler())
    , m_timestamp(getUnixTimeStamp())
    , m_is_ingress(true)
    , m_vect(masked_msg->m_vect)
{ }
...
DiscV5MaskedMessage::DiscV5MaskedMessage( const shared_ptr<const SessionHandler> session_handler,
                                          const uint32_t session_egress_msg_counter, const Flag flag,
...
{ 
    ...
    header.push_back(Network::GetInstance().getHostENR()->getID());     // src-id
    if( flag == Flag::HANDSHAKE)
    {
        ...
        header.push_back(Network::GetInstance().getHostENR()->getSignedRLP());
    }
...
const ByteStream DiscV5MaskedMessage::getMaskingKey() const
{
    ...
    if( m_is_ingress )
        masking_key = ByteStream(&Network::GetInstance().getHostENR()->getID()[0], 16);
    else
    {
                                                ...
                                                ByteStream &IDSignature ) const
{
    ...
    ByteStream node_id_a(Network::GetInstance().getHostENR()->getPubKey().getKey(Pubkey::Format::XY).keccak256());
    ByteStream node_id_b(peer_pub_key.getKey(Pubkey::Format::XY).keccak256());

    Privkey ephemeral_secret = Privkey::generateRandom();

    if( ephemeral_secret.getSecret() > 0 )
    {
        ...
        if(retval > 0)
        {
            ...
            Signature sig(Network::GetInstance().getHostENR()->getSecret()->sign(id_signature_input.sha256()));
