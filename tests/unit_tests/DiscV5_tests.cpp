...
TEST(DiscV5_tests, DiscV5_ingress_ordinary_ping)
{
    ...
    ASSERT_EQ(a_ping_b_auth_msg->getPeerID(), node_a_enr->getID());
...
TEST(DiscV5_tests, DiscV5_p2p_ordinary_ping)
{
    ...
    ASSERT_EQ(a_ping_b_received_msg->getProtocol(), a_ping_b_sent_msg->getProtocol());
    ASSERT_EQ(a_ping_b_received_msg->getVersion(), a_ping_b_sent_msg->getVersion());
    ASSERT_EQ(a_ping_b_received_msg->getFlag(), a_ping_b_sent_msg->getFlag());
    ASSERT_EQ(a_ping_b_received_msg->getNonce(), a_ping_b_sent_msg->getNonce());
    ASSERT_EQ(a_ping_b_received_msg->getAuthDataSize(), a_ping_b_sent_msg->getAuthDataSize());
    ASSERT_EQ(a_ping_b_received_msg->getSourceID(), a_ping_b_sent_msg->getSourceID());
    
    ASSERT_EQ(a_ping_b_received_msg->getMessageData(), a_ping_b_sent_msg->getMessageData());
    ASSERT_EQ(a_ping_b_received_msg->getType(), a_ping_b_sent_msg->getType());
    
    ASSERT_EQ(a_ping_b_received_msg->getRLPPayload(), a_ping_b_sent_msg->getRLPPayload());
    ASSERT_EQ(a_ping_b_received_msg->getRequestID(), a_ping_b_sent_msg->getRequestID());
    ASSERT_EQ(a_ping_b_received_msg->getENRSeq(), a_ping_b_sent_msg->getENRSeq());
}
...
TEST(DiscV5_tests, DiscV5_ingress_whoareyou)
{
    ...
    //Msg received by b
    auto a_ping_b_unauth_msg = make_shared<const DiscV5UnauthMessage>( udp_server_b,
    ...
    ASSERT_EQ(a_ping_b_msg->getENRSeq(), 0);
}
...
TEST(DiscV5_tests, DiscV5_p2p_whoareyou)
{
    ...
    // On the receiving-side (b)
    auto a_way_b_received_unauth_msg = make_shared<const DiscV5UnauthMessage>( udp_server_b,
                                                                               *a_way_b_sent_msg.get(),
                                                                               node_a_enr->getUDPAddress());
    auto a_way_b_received_msg = make_shared<const DiscV5WhoAreYouMessage>(a_way_b_received_unauth_msg);
    ...
    ASSERT_EQ(a_way_b_received_msg->getProtocol(), a_way_b_sent_msg->getProtocol());
    ASSERT_EQ(a_way_b_received_msg->getVersion(), a_way_b_sent_msg->getVersion());
    ASSERT_EQ(a_way_b_received_msg->getFlag(), a_way_b_sent_msg->getFlag());
    ASSERT_EQ(a_way_b_received_msg->getNonce(), a_way_b_sent_msg->getNonce());
    ASSERT_EQ(a_way_b_received_msg->getAuthDataSize(), a_way_b_sent_msg->getAuthDataSize());
    ASSERT_EQ(a_way_b_received_msg->getIDNonce(), a_way_b_sent_msg->getIDNonce());
    ASSERT_EQ(a_way_b_received_msg->getENRSeq(), a_way_b_sent_msg->getENRSeq());
}
...
TEST(DiscV5_tests, DiscV5_ingress_handshake_ping)
{
    ...
    auto b_session = make_shared<DiscV5Session>(udp_server_b, node_a_enr->getUDPAddress(), node_a_enr->getID());
    b_session->updatePeerENR(node_a_enr);
    b_session->setChallengeData(ByteStream("0x000000000000000000000000000000006469736376350001010102030405060708090a0b0c00180102030405060708090a0b0c0d0e0f100000000000000001", 63, 16));
    auto a_ping_b_unauth_msg = make_shared<DiscV5UnauthMessage>( udp_server_b,
...
TEST(DiscV5_tests, DiscV5_ingress_handshake_ping_with_enr)
{
    ...
    ASSERT_EQ(a_ping_b_auth_msg->getENR()->getName(), "-H24QBfhsHORjaMtZAZCx2LA4ngWmOSXH4qzmnd0atrYPwHnb_yHTFkkgIu-fFCJCILCuKASh6CwgxLR1ToX1Rf16ycBgmlkgnY0gmlwhH8AAAGJc2VjcDI1NmsxoQMT0UIR4Ch7I2GhYViQqbUhIIBUbQoleuTP-Wz1NJksuQ");
    ASSERT_EQ(a_ping_b_auth_msg->getENR()->getScheme(), "v4");
    ASSERT_EQ(a_ping_b_auth_msg->getENR()->getPubKey(), Pubkey(ByteStream("0x13d14211e0287b2361a1615890a9b5212080546d0a257ae4cff96cf534992cb97e6adeb003652e807c7f2fe843e0c48d02d4feb0272e2e01f6e27915a431e773", 64, 16), Pubkey::Format::XY));
    ASSERT_EQ(a_ping_b_auth_msg->getENR()->getID(), node_a_enr->getID());
    ASSERT_EQ(a_ping_b_auth_msg->getENR()->getSeq(), 1);
    ASSERT_EQ(a_ping_b_auth_msg->getENR()->getIP(), 0x7F000001);
    ASSERT_EQ(a_ping_b_auth_msg->getENR()->getUDPPort(), 24929);
    ASSERT_EQ(a_ping_b_auth_msg->getENR()->getTCPPort(), 12900);
...
TEST(DiscV5_tests, DiscV5_p2p_handshake_ping_with_enr)
{
    ...
    auto a_session = make_shared<DiscV5Session>(udp_server_a, node_b_enr->getUDPAddress(), node_b_enr->getID());
    a_session->updatePeerENR(node_b_enr);
    a_session->setChallengeData(ByteStream("0x000000000000000000000000000000006469736376350001010102030405060708090a0b0c00180102030405060708090a0b0c0d0e0f100000000000000001", 63, 16));
    ...
    ASSERT_EQ(a_ping_b_received_msg->getAuthDataSize(), a_ping_b_sent_msg->getAuthDataSize());
    ASSERT_EQ(a_ping_b_received_msg->getSourceID(), a_ping_b_sent_msg->getSourceID());
    ASSERT_EQ(a_ping_b_received_msg->getIDSignatureSize(), a_ping_b_sent_msg->getIDSignatureSize());
    ASSERT_EQ(a_ping_b_received_msg->getEphemeralPubKeySize(), a_ping_b_sent_msg->getEphemeralPubKeySize());
    ASSERT_EQ(a_ping_b_received_msg->getIDSignature(), a_ping_b_sent_msg->getIDSignature());
    ASSERT_EQ(a_ping_b_received_msg->getEphemeralPubKey(), a_ping_b_sent_msg->getEphemeralPubKey());
    ASSERT_EQ(a_ping_b_received_msg->getENR()->getSignedRLP(), a_ping_b_sent_msg->getENR()->getSignedRLP());
