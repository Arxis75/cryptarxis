...
TEST(RFC6979Tests, RFC6979_NIST_P256)
{
    ...
    Privkey x(ByteStream("0xC9AFA9D845BA75166B5C215767B1D6934E50C3DB36E89B127B8A622B120F6721", 32, 16), ecc);
    Pubkey U = x.getPubKey();
    
    Integer expected = ByteStream("60FED4BA255A9D31C961EB74C6356D68C049B8923B61FA6CE669622E60F29FB6", 32, 16);
    Integer actual = U.getPoint().getX();
    ASSERT_EQ(actual, expected);
    expected = ByteStream("7903FE1008B8BC99A41AE9E95628BC64F2F1B20C2D7E9F5177A3C294D4462299", 32, 16);
    actual = U.getPoint().getY();
    ASSERT_EQ(actual, expected);

    const char *message = "sample";
    ByteStream t_raw(message,strlen(message));
    ByteStream t_h(t_raw.sha256());

    expected = ByteStream("A6E3C57DD01ABE90086538398355DD4C3B17AA873382B0F24D6129493D8AAD60", 32, 16);
    actual = ecc.generate_RFC6979_nonce(x.getSecret(), t_h, 0);
    ASSERT_EQ(actual, expected);

    Signature expected_signature( ByteStream("EFD48B2AACB6A8FD1140DD9CD45E81D69D2C877B56AAF991C34D0EA84EAF3716", 32, 16),
                                  ByteStream("F7CB1C942D657C41D436C7A1B6E29F65F3E900DBB9AFF4064DC4AB2F843ACDA8", 32, 16),
                                  false );
    ...
    t_raw = ByteStream(message,strlen(message));
    t_h = ByteStream(t_raw.sha256());

    expected = ByteStream("D16B6AE827F17175E040871A1C7EC3500192C4C92677336EC2537ACAEE0008E0", 32, 16);
    actual = ecc.generate_RFC6979_nonce(x.getSecret(), t_h, 0);
    ASSERT_EQ(actual, expected);

    expected_signature = Signature( ByteStream("F1ABB023518351CD71D881567B1EA663ED3EFCF6C5132B354F28D3B0B7D38367", 32, 16),
                                    ByteStream("019F4113742A2B14BD25926B49C649155F267E60D3814B4C0CC84250E46F0083", 32, 16),
                                    false );
...
TEST(RFC6979Tests, Micah_sign_vectors)
{
    ...
    Signature expected_signature( ByteStream("433EC3D37E4F1253DF15E2DEA412FED8E915737730F74B3DFB1353268F932EF5", 32, 16),
                                  ByteStream("557C9158E0B34BCE39DE28D11797B42E9B1ACB2749230885FE075AEDC3E491A4", 32, 16),
                                  false );
    ...
    x = ByteStream("0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEBAAEDCE6AF48A03BBFD25E8CD0364140", 32, 16);
    expected_signature = Signature( ByteStream("45CEA25D72DB4929DC27BC66527BBB215D20E323FF0DE944640930BE5C38C534", 32, 16),
                                    ByteStream("34F8904BDE08FB97BE5D01C6BC5AF0189FD76E0E03693E56DAB28BFCD956F150", 32, 16),
                                    true );
